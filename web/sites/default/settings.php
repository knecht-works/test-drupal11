 ...
 * @endcode
 */
$settings['hash_salt'] = 'CO3X-uZQF3jdmYFETSm1CA5mPNN7YHIf-NvkA4wjImBUUhQVQQoHXck5mV-cdL3lCm25hKQI2Q';
...
/**
 * Knecht test-drupal11 fixture settings.
 *
 * This file is committed and intentionally generic. Database credentials come
 * from DDEV's auto-generated settings.ddev.php (not tracked in git); the values
 * below are fallbacks so the fixture also boots outside of DDEV.
 */
$settings['config_sync_directory'] = '../config/sync';

// Load DDEV-managed settings (database credentials, file paths, etc.).
if (getenv('IS_DDEV_PROJECT') == 'true' && file_exists(__DIR__ . '/settings.ddev.php')) {
  include __DIR__ . '/settings.ddev.php';
}

// Static hash salt so the fixture behaves the same on every checkout.
$settings['hash_salt'] = $settings['hash_salt'] ?? 'knecht-test-drupal11-e2e-fixture-hash-salt';

// This is a throwaway test fixture: accept any Host header so the site also
// works behind whatever hostname Knecht proxies it on.
$settings['trusted_host_patterns'] = ['.*'];
